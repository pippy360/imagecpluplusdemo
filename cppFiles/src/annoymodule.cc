...
public:
    ...
    int64_t _f_external, _f_internal;
  AnnoyIndex<int64_t, uint64_t, ::Hamming, Kiss64Random> _index;
  void _pack(const float* src, uint64_t* dst) const {
	for (int64_t i = 0; i < _f_internal; i++) {
	  dst[i] = 0;
	  for (int64_t j = 0; j < 64 && i*64+j < _f_external; j++) {
